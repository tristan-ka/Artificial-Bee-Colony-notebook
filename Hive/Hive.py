...
class Bee(object):
    ...
    def _random(self, lower, upper):
        ...
        self.vector = [0]
        path=[i for i in range(1,len(upper))]
        random.shuffle(path)
        self.vector+=path
...
class BeeHive(object):
    ...
    def run(self):
        ...
        for itr in range(self.max_itrs):
            ...
            if int(self.best)<=int(self.bestDist) : break
    ...
    def find_best(self):
        ...
        if (values[index] < self.best):
            ...
            self.record.append(self.solution)
...
#        while (bee_ix == index): bee_ix = random.randint(0, self.size-1)
        ...
        zombee.vector = self._mutate(index)

#        # produces a mutant based on current bee and bee's friend
#        zombee.vector = self._mutate(index,bee_ix)
    ...
    def _mutate(self, current_bee):
        ...
        """
        path=copy.copy(self.population[current_bee].vector[1:]) 
        l=len(path)
...
#    def _mutate(self, current_bee, other_bee):
#        """
#
#        Mutates a given solution vector - i.e. for continuous
#        real-values.
